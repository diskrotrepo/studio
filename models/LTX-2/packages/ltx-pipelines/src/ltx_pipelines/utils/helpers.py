...
def denoise_audio_only(  # noqa: PLR0913
    output_shape: VideoPixelShape,
    noiser: Noiser,
    sigmas: torch.Tensor,
    stepper: DiffusionStepProtocol,
    ...
    """Denoise audio only, with a frozen dummy video state.

    Mirrors :func:`denoise_video_only` but inverted: the audio state is actively
    denoised while the video state is frozen (noise_scale=0, denoise_mask=0).
    The video dimensions are kept minimal since no video is decoded.
    """
    # Frozen dummy video state — minimal dimensions, no denoising
    dummy_video_shape = VideoPixelShape(
        batch=output_shape.batch,
    ...
    video_state, _ = noise_video_state(
        output_shape=dummy_video_shape,
        noiser=noiser,
        conditionings=[],
        components=components,
        dtype=dtype,
...
def simple_audio_only_denoising_func(
    audio_context: torch.Tensor, transformer: X0Model
) -> DenoisingFunc:
    """Create a denoising function for audio-only generation.

    Passes ``video=None`` to the transformer (required for AudioOnly model type)
    and returns the video state's clean latent as a dummy to satisfy the
    :class:`DenoisingFunc` protocol.
    """

    def audio_only_denoising_step(
        video_state: LatentState, audio_state: LatentState, sigmas: torch.Tensor, step_index: int
    ) -> tuple[torch.Tensor, torch.Tensor]:
        sigma = sigmas[step_index]
        pos_audio = modality_from_latent_state(audio_state, audio_context, sigma)
