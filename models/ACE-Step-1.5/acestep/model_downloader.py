    ...
    for src_file in source_dir.glob("*.py"):
        ...
        try:
            ...
            if dst_file.exists() and _file_hash(src_file) == _file_hash(dst_file):
                logger.debug(f"[Model Sync] {src_file.name} already up-to-date (read-only target)")
            else:
                logger.warning(
                    f"[Model Sync] Permission denied writing {dst_file} "
                    f"and file {'differs from' if dst_file.exists() else 'missing at'} source. "
                    f"Fix with: chmod -R a+w {target_dir}"
                )
...
MAIN_MODEL_REPO = "diskrot/Ace-Step1.5-diskrot"
...
SUBMODEL_REGISTRY: Dict[str, str] = {
    ...
    "acestep-v15-turbo-shift1": "diskrot/acestep-v15-turbo-shift1-diskrot",
    "acestep-v15-turbo-continuous": "diskrot/acestep-v15-turbo-continuous-diskrot",
}
