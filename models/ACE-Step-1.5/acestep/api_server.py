...
MODEL_REPO_MAPPING = {
    ...
    "acestep-v15-turbo": "diskrot/Ace-Step1.5-diskrot",
    "acestep-5Hz-lm-1.7B": "diskrot/Ace-Step1.5-diskrot",
    "vae": "diskrot/Ace-Step1.5-diskrot",
    "Qwen3-Embedding-0.6B": "diskrot/Ace-Step1.5-diskrot",
    # Separate model repositories
    "acestep-5Hz-lm-0.6B": "diskrot/acestep-5Hz-lm-0.6B-diskrot",
    "acestep-5Hz-lm-4B": "diskrot/acestep-5Hz-lm-4B-diskrot",
    "acestep-v15-base": "diskrot/acestep-v15-base-diskrot",
    "acestep-v15-sft": "diskrot/acestep-v15-sft-diskrot",
    "acestep-v15-turbo-shift3": "diskrot/acestep-v15-turbo-shift3-diskrot",
}

DEFAULT_REPO_ID = "diskrot/Ace-Step1.5-diskrot"
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            import tempfile
            fallback = os.path.join(tempfile.gettempdir(), "acestep_api_audio")
            os.makedirs(fallback, exist_ok=True)
            app.state.temp_audio_dir = fallback
            logger.warning(f"[API Server] Cannot write to {tmp_root}, using fallback: {fallback}")
        ...
        async def _run_one_job(job_id: str, req: GenerateMusicRequest) -> None:
            ...
            def _blocking_generate() -> Dict[str, Any]:
                ...
                return {
                    ...
                    "lora": {
                        "active": getattr(h, "use_lora", False),
                        "adapter": getattr(h, "_lora_active_adapter", None) if getattr(h, "use_lora", False) else None,
                        "scale": getattr(h, "lora_scale", 1.0) if getattr(h, "use_lora", False) else None,
                    },
                }
    ...
    def _get_loras_dir() -> str:
        """Return the loras directory (project_root/loras)."""
        return os.path.join(_get_project_root(), "loras")

    def _resolve_lora_path(lora_path: str) -> str:
        """Resolve a LoRA path: if it's a bare name, look it up in the loras directory."""
        # If it's already an absolute path or relative path that exists, use as-is
        if os.path.sep in lora_path or os.path.exists(lora_path):
            return lora_path
        # Treat as a name: look for loras/<name>/final
        candidate = os.path.join(_get_loras_dir(), lora_path, "final")
        if os.path.isdir(candidate):
            return candidate
        # Fall back to loras/<name> directly
        candidate = os.path.join(_get_loras_dir(), lora_path)
        if os.path.isdir(candidate):
            return candidate
        # Return original — let the handler produce the error
        return lora_path

    @app.get("/v1/lora/list")
    async def list_available_loras(_: None = Depends(verify_api_key)):
        """List all LoRA adapters available on disk and their loaded status."""
        loras_dir = _get_loras_dir()
        handler: AceStepHandler = app.state.handler
        loaded_adapters = []
        active_adapter = None
        if handler and handler.model is not None:
            status = handler.get_lora_status()
            loaded_adapters = status.get("adapters", [])
            active_adapter = status.get("active_adapter")

        available = []
        if os.path.isdir(loras_dir):
            for name in sorted(os.listdir(loras_dir)):
                lora_dir = os.path.join(loras_dir, name)
                if not os.path.isdir(lora_dir):
                    continue
                # Check for valid adapter (final/ subdir or direct adapter_config.json)
                final_dir = os.path.join(lora_dir, "final")
                has_peft = os.path.exists(os.path.join(final_dir, "adapter_config.json")) or os.path.exists(os.path.join(lora_dir, "adapter_config.json"))
                has_lokr = os.path.exists(os.path.join(final_dir, "lokr_weights.safetensors")) or os.path.exists(os.path.join(lora_dir, "lokr_weights.safetensors"))
                if not has_peft and not has_lokr:
                    continue
                available.append({
                    "name": name,
                    "loaded": name in loaded_adapters,
                    "active": name == active_adapter,
                    "type": "lokr" if has_lokr else "lora",
                })

        return _wrap_response({"loras": available})

    @app.post("/v1/lora/load")
    async def load_lora_endpoint(request: LoadLoRARequest, _: None = Depends(verify_api_key)):
        """Load LoRA adapter into the primary model. lora_path can be a full path or a name from /v1/lora/list."""
        handler: AceStepHandler = app.state.handler
        ...
        try:
            resolved_path = _resolve_lora_path(request.lora_path.strip())
            adapter_name = request.adapter_name.strip() if isinstance(request.adapter_name, str) else None
            if adapter_name:
                result = handler.add_lora(resolved_path, adapter_name=adapter_name)
            else:
                result = handler.load_lora(resolved_path)

            if result.startswith("✅"):
                response_data = {"message": result, "lora_path": resolved_path}
                if adapter_name:
    ...
    @app.get("/v1/audio")
    async def get_audio(path: str, request: Request):
