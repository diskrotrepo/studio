...
def register_training_dataset_routes(
    ...
    async def upload_dataset_zip(
        ...
        dataset_name: str = Form("my_lora_dataset"),
        custom_tag: str = Form(""),
        tag_position: str = Form("replace"),
        all_instrumental: bool = Form(True),
        _: None = Depends(verify_api_key),
    ):
        """Upload a zip file containing mp3 files and a dataset.json."""

        from acestep.training.dataset_builder import DatasetBuilder

        if not file.filename or not file.filename.lower().endswith(".zip"):
            return wrap_response(None, code=400, error="File must be a .zip archive")

        extract_dir = None
        try:
            # Read zip bytes into a temp file
            ...
            try:
                ...
                if not zipfile.is_zipfile(tmp_zip.name):
                    return wrap_response(None, code=400, error="Uploaded file is not a valid zip archive")

                # Extract to a persistent directory under uploads/
                uploads_root = os.path.join(os.getcwd(), "uploads", "datasets")
                os.makedirs(uploads_root, exist_ok=True)
                extract_dir = os.path.join(uploads_root, str(uuid4()))
                ...
                with zipfile.ZipFile(tmp_zip.name, "r") as zf:
                    ...
                    for member in zf.namelist():
                        ...
                        if not resolved.startswith(os.path.realpath(extract_dir)):
                            return wrap_response(None, code=400, error=f"Zip contains unsafe path: {member}")
            ...
            dataset_json_path = os.path.join(extract_dir, "dataset.json")
            if not os.path.isfile(dataset_json_path):
                return wrap_response(None, code=400, error="Zip must contain a dataset.json file")

            # Validate: all audio files must be .mp3
            non_mp3 = []
            for root, _dirs, files in os.walk(extract_dir):
                for fname in files:
                    ext = os.path.splitext(fname)[1].lower()
                    # Skip non-audio files (json, txt, csv are allowed as metadata)
                    if ext in {".json", ".txt", ".csv", ".caption", ".lyrics"}:
                        continue
                    if ext != ".mp3":
                        non_mp3.append(fname)

            if non_mp3:
                examples = ", ".join(non_mp3[:5])
                suffix = f" (and {len(non_mp3) - 5} more)" if len(non_mp3) > 5 else ""
                return wrap_response(
                    None,
                    code=400,
                    error=f"All audio files must be .mp3. Found non-mp3 files: {examples}{suffix}",
                )
            ...
            samples, status = builder.load_dataset(dataset_json_path)

            if not samples:
                return wrap_response(None, code=400, error=status)

            # Rewrite audio_path to point to extracted files
            for sample in builder.samples:
                if sample.audio_path and not os.path.isabs(sample.audio_path):
                    sample.audio_path = os.path.join(extract_dir, sample.audio_path)
                elif sample.filename:
                    candidate = os.path.join(extract_dir, sample.filename)
                    if os.path.isfile(candidate):
                        sample.audio_path = candidate
            ...
            app.state.dataset_builder = builder
            app.state.dataset_json_path = dataset_json_path

            return wrap_response(
                {
                    "message": f"✅ Uploaded and loaded {len(samples)} samples from zip",
            ...
            logger.exception("Dataset zip upload failed")
            return wrap_response(None, code=500, error=f"Upload failed: {exc}")
